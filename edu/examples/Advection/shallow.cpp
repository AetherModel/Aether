...
arma_vec init_height(int64_t nPts) {

  arma_vec height(nPts);
  for (int64_t i = 0; i < nPts; i++) {
...
int main() {
  ...
  arma_vec height = init_height(nPtsTotal);
  arma_vec vel = init_vel(nPtsTotal);
  arma_vec hu = height % vel;
  arma_vec diff;

  exchange(height, nPts, nGCs);
  exchange(vel, nPts, nGCs);

  hu = height % vel;
  
  output(height, "height.txt", false, nPts, nGCs);
  output(vel, "vel.txt", false, nPts, nGCs);
  
  for (iStep = 0; iStep < nSteps; iStep++) {
  
    grad_height = calc_grad(height, x, nPts, nGCs);

    // Right side of edge from left
    heightR = project_from_left(height, grad_height,
			     x, edges,
			     nPts, nGCs);
    
    // Left side of edge from left
    heightL = project_from_right(height, grad_height,
			      x, edges,
			      nPts, nGCs);
    ...
    eq1FluxL = heightL % velL;
    eq1FluxR = heightR % velR;

    eq2FluxL = heightL % velL % velL + g/2 * heightL % heightL;
    eq2FluxR = heightR % velR % velR + g/2 * heightR % heightR;

    // Calculate the wave speed for the diffusive flux:
    wsL = abs(velL) + sqrt(g * heightL);
    wsR = abs(velR) + sqrt(g * heightR);
    ws = wsL;
    ...
    diff = eq1FluxL - eq1FluxR;
    eq2Flux = (eq2FluxL + eq2FluxR) / 2 + 0.5 * ws % diff;

    // Update values:
    for (int64_t i = nGCs; i < nPts + nGCs*2 - 1; i++) {
      height(i) = height(i) + dt / widths(i) * (eq1Flux(i+1) - eq1Flux(i));
      hu(i) = hu(i) + dt / widths(i) * (eq2Flux(i+1) - eq2Flux(i));
    }
    
    exchange(height, nPts, nGCs);
    exchange(hu, nPts, nGCs);
    vel = hu / height;

    output(height, "height.txt", true, nPts, nGCs);
    output(vel, "vel.txt", true, nPts, nGCs);
    //output(height, "test.txt", true, nPts, nGCs);
    //output(heightL, "heightr.txt", false, nPts, nGCs);
    //output(heightR, "heightl.txt", false, nPts, nGCs);
  }
