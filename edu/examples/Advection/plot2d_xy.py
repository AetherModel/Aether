...
def plot_data(values, fileout, mini, maxi, x, y):

    fig = plt.figure(figsize = (10,10))
    ax = fig.add_subplot(111)
    cmap = cm.plasma if mini >= 0 else cm.bwr
    #ax.scatter(x, y, c = values, vmin = mini, vmax = maxi, cmap = cmap)
    ax.pcolor(values, vmin = mini, vmax = maxi, cmap = cmap)
...
for i in range(0, n, nSkip):
    ...
    plot_data(v, fileout, mini, maxi, x, y)
