...
/*
  ...
  to compile:
  g++ -I/usr/local/include -I/Users/ridley/Software/Json/json/include -o sphere2d_fd sphere2d_fd.cpp
...
/* This is part of Aether now, so don't need to define here:
...
void exchange(arma_mat &values, int64_t nGCs, bool reverse) {
  ...
  for (int64_t j = 0; j < nGCs; j++) {
    for (int64_t i = 0; i < nX; i++) {
      ii = (i + nX/2) % nX;
      // South pole:
      values(i, j) = s * copy(ii, 2*nGCs - 1 - j);
      // North pole:
      values(i, nY - 1 - j) = s * copy(ii, nY - 1 - 2*nGCs + j);
    }
  }
...
projection_struct project_to_edges(arma_mat &values,
  ...
  // Right side projected from the right
  ...
  proj.grad_edge_LR.resize(nX, nY);
  proj.grad_edge_DU.resize(nX, nY);

  for (int64_t j = 1; j < nY - 1; j++) {
    for (int64_t i = 1; i < nX - 1; i++) {
      proj.grad_edge_LR(i, j) =
	(proj.R(i+1, j) + proj.L(i+1, j)) -
	(proj.R(i, j) + proj.L(i, j)) /
	(x_edges(i+1, j) - x_edges(i, j));
      proj.grad_edge_DU(i, j) =
	(proj.U(i, j+1) + proj.U(i, j+2)) -
	(proj.D(i, j) + proj.D(i, j)) /
	(y_edges(i, j+1) - y_edges(i, j));
    }
  }
...
int main() {
  ...
  exchange(rho, nGCs, false);
  exchange(xVel, nGCs, true);
