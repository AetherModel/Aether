...
contains
  ...
  ! ------------------------------------------------------------------------
   ...
   subroutine run_fta_model(EFluxOut, AveEOut, iError)
      ...
      do iLon = 1, IOi_NeednMLTs
         do iLat = 1, IOi_NeednLats

            iMlt = mod(floor(mod(IOr2_NeedMLTs(iLon, iLat)+24.0,24.0)*4),nMltsFta)
            if (iMlt == 0) iMlt = nMltsFta

            gMlat = abs(IOr2_NeedLats(iLon, iLat))
            if (gMlat > minLat) then
               iMlat = (gMlat - minLat) / dLat
               EFluxOut(iLon, iLat) = eFlux(iMlt, iMlat)
               AveEOut(iLon, iLat) = AveE(iMlt, iMlat)
            endif
