...
C       CHARACTER*30 Copyright
	...
	REAL Coef(0:1,0:5,0:5),BoundFit(0:1,0:5),pi
	double precision d_fsc(0:1,0:4)
	DOUBLE PRECISION dpi, omega
	INTEGER*4 i,j,k,l,m,n,o
	INTEGER*4 maxj,MaxL,MaxM,MaxN,MaxO
	COMMON /EIE_AllW01Coefs/MaxJ,MaxO,CS,BCS,SS,BSS
	COMMON /EIE_SetW01Coef/MaxL,MaxM,MaxN,Coef,BoundFit,pi,dpi
	    ...
	    DO m=0,mlimit
		  ...
		  DO k=0,klimit
			...
			DO j=0,MaxJ
			    ...
			    ENDDO
				d_fsc = fsc
     			    acoef=acoef+ XA(j)*FSVAL(omega,MaxO,d_fsc)
			ENDDO
			IF(UseAL)THEN
			    DO j=0,1
				  DO o=0,MaxO
	...
	DO n=0,MaxN
	    ...
	    DO k=0,klimit
		  ...
		  DO j=0,MaxJ
			...
			ENDDO
			d_fsc = fsc
     			acoef=acoef+ XB(j)*FSVAL(omega,MaxO,d_fsc)
		  ENDDO
		  IF(UseAL)THEN
			DO j=0,1
			    DO o=0,MaxO
...
****************** Copyright 1996, 2001, Dan Weimer/MRC ***********************
	...
	REAL gmlt
	REAL Coef(0:1,0:5,0:5),BoundFit(0:1,0:5),pi
	DOUBLE PRECISION dpi, d_gmlt, d_boundfit(0:1,0:5)
	INTEGER MaxL,MaxM,MaxN
	COMMON /EIE_SetW01Coef/MaxL,MaxM,MaxN,Coef,BoundFit,pi,dpi
	d_gmlt = gmlt * pi / 12
	d_boundfit = BoundFit
	BoundaryLat01=FSVal(d_gmlt,MaxN,d_boundfit)
	RETURN
...
******************************************************************************
      ...
      IF (IYYY.EQ.0) write(*,*) 'There is no Year Zero.'
